...
import {NavigationBarProps, SystemBarStyle} from "./types";
...
export class NavigationBar extends React.Component<NavigationBarProps> {
  ...
  static currentHeight: number =
    Platform.OS === 'android'
      ? NativeModule?.getConstants().navigationBarHeight
      : null;

  static setBarStyle(style: SystemBarStyle) {
    const oldProps = NavigationBar.mergedProps;
    if (style === "light-content" || style === "dark-content") {
      if (isSupportedPlatform && oldProps?.barStyle !== style) {
        NativeModule?.setNavigationBarStyle(style);
        const newEntry = createStackEntry({barStyle: style});
        NavigationBar.mergedProps = {...newEntry};
      }
    }
  }
